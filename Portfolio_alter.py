...
class Portfolio:
    def __init__(self, tickers, iniNAV, iniprice, portweight: pd.DataFrame, inidate, leverage):
        ...
        self.margin = self.groupNAV.to_numpy() * 0.3 # -self.values[self.values[2] < 0].to_numpy()[:,2] // self.lev    # update if change in portweight
    ...
    def __calShare__(self, industry):
        ...
        if wA > 0:
            long = int(nav * 0.7 / PA)
            short = int(wB * long)
            shares = np.array([long, short])
        else:
            long = int(nav * 0.7 / PB)
            short = int(wA * long)
            shares = np.array([short, long])
